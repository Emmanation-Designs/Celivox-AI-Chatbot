...
const LiveOverlay: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 flex flex-col items-center justify-between py-12 px-4 animate-fade-in">
      ...
      <div className="relative flex items-center justify-center w-64 h-64">
        ...
        <div className={`relative z-10 w-32 h-32 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 transform 
      ...
      <div className="w-full max-w-md text-center space-y-8">
        <div className="h-16 flex items-center justify-center px-4">
           {transcript && (
             <p className="text-yellow-300 text-lg font-medium leading-relaxed animate-fade-in">
               "{transcript}"
             </p>
           )}
        </div>
        ...
        <div className="flex items-center justify-center gap-8">
           <button className="p-4 rounded-full bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white transition-all cursor-not-allowed opacity-50">
              <MicOff size={24} />
           </button>
           <button 
             onClick={onClose}
             className="p-6 rounded-full bg-red-500 text-white hover:bg-red-600 shadow-lg shadow-red-500/30 transform hover:scale-105 transition-all"
           >
              <PhoneOff size={32} />
           </button>
           <button 
             onClick={onSettingsClick}
             className="p-4 rounded-full bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white transition-all z-[60] relative"
           >
              <Settings size={24} />
           </button>
        </div>
...
export default function App() {
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'} flex h-screen font-sans overflow-hidden relative transition-colors duration-300`}>
          ...
          {topics.map(topic => (
            <div key={topic.id} className="mb-1 group">
                <div className={`flex flex-col rounded-lg transition-colors ${currentTopicId === topic.id ? (isDarkMode ? 'bg-gray-700' : 'bg-gray-200') : 'hover:bg-gray-700/50'}`}>
                   <button 
                      onClick={() => {
                        // If clicking the active one, toggle menu. If clicking new one, select and open menu.
      ...
      <div className="flex-1 flex flex-col h-full relative">
        <div className={`h-16 border-b flex items-center px-4 justify-between backdrop-blur supports-[backdrop-filter]:bg-opacity-80 ${isDarkMode ? 'border-gray-700 bg-gray-900/95 text-white' : 'border-gray-200 bg-white/95 text-gray-900'}`}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-medium text-lg">{topics.find(t => t.id === currentTopicId)?.title || 'New Conversation'}</h2>
          </div>
          <div className="flex items-center gap-2">
             {isSpeaking && !isLiveMode && <div className="flex items-center gap-1 text-green-400 text-sm mr-2 animate-pulse"><Volume2 size={16} /> Speaking...</div>}
        ...
        <div className={`flex-1 overflow-y-auto p-4 space-y-6 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
          {messages.length === 0 && (
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''} group`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-blue-600' : 'bg-green-600 text-white'}`}>{msg.role === 'user' ? (user.displayName ? user.displayName[0] : 'U') : <Sparkles size={16} />}</div>
              <div className={`max-w-[80%] space-y-1 flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div 
                  className="cursor-pointer transition-all"
                  onClick={() => setActiveMessageId(activeMessageId === msg.id ? null : msg.id)}
                >
                  {msg.image && (
                     <img src={msg.image} alt="Content" className="rounded-lg max-w-xs shadow-lg border border-gray-700 mb-2" />
                  )}
                  <div className={`px-4 py-3 rounded-2xl shadow-sm relative ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-sm' : (isDarkMode ? 'bg-gray-800 text-gray-100 border border-gray-700' : 'bg-gray-100 text-gray-900 border border-gray-200') + ' rounded-tl-sm'}`}>
        ...
        <div className={`p-2 md:p-4 border-t ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
           {attachment && (
             <div className={`flex items-center gap-2 mb-2 p-2 rounded-lg w-fit ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}><div className={`text-xs truncate max-w-[200px] ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{attachment.name}</div><button onClick={() => setAttachment(null)} className="text-gray-500 hover:text-red-500"><X size={14}/></button></div>
           )}
           
           <div className={`relative flex items-center gap-1 md:gap-2 rounded-xl border px-2 py-2 focus-within:ring-2 focus-within:ring-blue-500 transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'}`}>
              ...
              <div className="flex items-center shrink-0 gap-0.5 md:gap-1">
                 <button type="button" onClick={() => fileInputRef.current?.click()} className={`p-1.5 md:p-2 rounded-lg transition-colors ${isDarkMode ? 'text-gray-400 hover:text-white hover:bg-gray-700' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}><Paperclip size={20} /></button>
                 <button type="button" onClick={toggleImageGen} className={`p-1.5 md:p-2 rounded-lg transition-colors ${isDarkMode ? (input.toLowerCase().startsWith('/imagine') ? 'text-purple-400 bg-purple-500/20' : 'text-gray-400 hover:text-white hover:bg-gray-700') : (input.toLowerCase().startsWith('/imagine') ? 'text-purple-600 bg-purple-100' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100')}`} title="Generate Image"><ImageIcon size={20} /></button>
                 <button type="button" onClick={handleDictation} className={`p-1.5 md:p-2 rounded-lg transition-colors ${isDarkMode ? 'text-gray-400 hover:text-white hover:bg-gray-700' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`} title="Dictation (Text Mode)"><Mic size={20} /></button>
              </div>
              <input 
                value={input}
              ...
              <div className="flex items-center gap-1 shrink-0">
                 <button type="button" onClick={startLiveMode} className="p-1.5 md:p-2 rounded-lg text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 transition-all" title="Live Mode"><Headphones size={22} /></button>
                 <button type="button" onClick={() => handleSend()} disabled={!input.trim() && !attachment} className="p-1.5 md:p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"><Send size={18} /></button>
              </div>
           </div>
        </div>
      </div>
